...
```{r}
#Now I'm going to repeat that for my second sample
PEM=read.csv("Plant-O-Matic list.csv")
head(PEM)
tail(PEM)
speciesID=paste(PEM$Scientific, PEM$Name, sep=" ")
speciesID
PEM_list=count(speciesID)
head(PEM_list)
PEM_list=data.frame("PEM", PEM_list)
names(PEM_list)<-c("ListID", "SpeciesID", "Freq")
```
```{r}
Plants2=rbind(PEM_list, MH_list)
dim(Plants2)
wab=matrify(PLants2)
wab
```
