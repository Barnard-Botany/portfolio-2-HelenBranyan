...
#here is my martix for yellowstone park 

```{r}
park <- BIEN_occurrence_county(country="United States", state="Wyoming", county = "Park")
head(park)
tail(park)
gallatin <- BIEN_occurrence_county(country="United States", state="Montana", county = "Gallatin")
head(gallatin)
tail(gallatin)
attributes(gallatin)
fremont <- BIEN_occurrence_county(country="United States", state="Idaho", county = "Fremont")
head(fremont)
tail(fremont)
attributes(fremont)
```
...
park.occ=park[c(1)]
head(park.occ)
gallatin.occ=gallatin[c(1)]
head(gallatin.occ)
fremont.occ=fremont[c(1)]
head(fremont.occ)
park.ab=count(park.occ)
head(park.ab)
gallatin.ab=count(gallatin.occ)
head(gallatin.ab)
fremont.ab=count(fremont.occ)
head(fremont.ab)
park.ab=data.frame("park", park.ab)
head(park.ab)
names(park.ab)<-c("ListID", "SpeciesID", "Freq")
head(park.ab)
gallatin.ab=data.frame("gallatin", gallatin.ab)
head(gallatin.ab)
names(gallatin.ab)<-c("ListID", "SpeciesID", "Freq")
head(gallatin.ab)
fremont.ab=data.frame("fremont", fremont.ab)
head(fremont.ab)
names(fremont.ab)<-c("ListID", "SpeciesID", "Freq")
head(fremont.ab)
threecounty.ab=rbind(park.ab, gallatin.ab, fremont.ab)
head(threecounty.ab)
tail(threecounty.ab)
dim(threecounty.ab)
urban.ab<-matrify(threecounty.ab)
...
```
# it is not surpirising that there is quite a bit of overlap between the species occuring in the three counties, as they are physically very close to each other and the wildlife has been generally undistrubed by humans (national parks are well protected/preserved). However, there is less diversity in flora between park and gallatin than expected, especially for the Simpson's index (.36). 

#here is my matrix for a list of plants of Mannahatta and Plant-O-Matic

```{r}
MH=read.csv("Mannahatta list .csv")
head(MH)
tail(MH)
```
```{r}
...
```
```{r}
POM=read.csv("Plant-O-Matic list.csv")
head(POM)
tail(POM)
```
```{r}
POM_list=count(POM)
head(POM_list)
POM_list=data.frame(POM_list)
POM_list=data.frame("POM", POM_list)
head(POM_list)
```
```{r}
Helen2=rbind(POM_list, MH_list)
dim(Helen2)
wab=matrify(Helen2)
wab
```
# We can't matrify these data! There is no occurance data because each plant is only listed once. This isn't to say that this data is not helpful/we could not gain occurance data. Now that we have a list of all of the plants around 543 Wilcoxson Avenue, Stratford, CT (i.e the Plant-O-Matic data), we could go out in the field and count how many specimens of each we encounter. Similarly, a database such as BIEN may already have some of the occurance data, which we could then download and work with. 
